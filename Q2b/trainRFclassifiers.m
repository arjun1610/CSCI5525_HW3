function classifier = trainRFclassifiers( X, y, nFeatures)

%TRAINCLASSIFIERS Summary of this function goes here
%   Detailed explanation goes here

%first level split
[~, splitValue0, featureIndex0, xLeft0, xRight0, yLeft0, yRight0] = getRFeatureForSplit(X, y, 0, 0, nFeatures);
% second level split
if ~isempty(xLeft0) && ~isempty(xRight0) 
    % for left side
    [~, splitValue1, featureIndex1, ~, ~, yLeft1, yRight1] = getRFeatureForSplit(xLeft0, yLeft0, featureIndex0, 1, nFeatures);
    % assign class labels
    if featureIndex1 ==0
    ...
    % for right side 
    [~, splitValue2, featureIndex2, ~, ~, yLeft2, yRight2] = getRFeatureForSplit(xRight0, yRight0, featureIndex0, 1, nFeatures);
    % assign class labels 
    if featureIndex2 == 0
        %set pure class labels
